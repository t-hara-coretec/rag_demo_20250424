...
document.getElementById('clear-cache-btn').addEventListener('click', () => clearUrlCache().catch(onError))

// Function to clear the URL cache
async function clearUrlCache(): Promise<void> {
  if (confirm('Are you sure you want to clear the URL cache? This will remove all cached web content.')) {
    try {
      // Show loading state
      ...
      const response = await fetch('/clear_url_cache/', {method: 'POST'});
      ...
      if (data.status === 'success') {
        ...
        urlList.innerHTML = `
          <div class="alert alert-success py-1 mb-0">
            <small>Cache cleared successfully!</small>
          </div>
        `;
        
        // Refresh URLs list after a short delay
